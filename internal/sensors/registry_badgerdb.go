...
type badgerDBRegistry struct {
	db *badger.DB
}

func openSensorRegistry(ctx context.Context, registryPath fyne.URI) (*badgerDBRegistry, error) {
	var db *badger.DB
	var err error
	if registryPath != nil {
	...
	return &badgerDBRegistry{db: db}, nil
}

func (reg *badgerDBRegistry) Open(ctx context.Context, registryPath fyne.URI) error {
	return nil
}

func (reg *badgerDBRegistry) Close() error {
	if reg.db != nil {
		return reg.db.Close()
	} else {
		return nil
...
func (reg *badgerDBRegistry) Get(id string) (*sensorMetadata, error) {
...
func (reg *badgerDBRegistry) Set(id string, values *sensorMetadata) error {
