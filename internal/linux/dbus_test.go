...
func TestNewBusRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewBusRequest(tt.args.busType); !reflect.DeepEqual(got, tt.want) {
...
func Test_variantToValue(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "int test",
			args: args{variant: dbus.MakeVariant(4)},
			want: 4,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := variantToValue[int](tt.args.variant); !reflect.DeepEqual(got, tt.want) {
