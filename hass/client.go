...
func NewClient(ctx context.Context) (*Client, error) {
	...
	client := &Client{
		...
		haConfig:       &Config{ConfigResponse: &api.ConfigResponse{}},
	...
	if err := config.Load(hassConfigPrefix, client.clientConfig); err != nil {
		return nil, fmt.Errorf("unable to load agent config: %w", err)
	}
...
func (c *Client) isDisabledInHA(id models.UniqueID) (bool, error) {
	status, err := c.haConfig.IsEntityDisabled(id)
	if err != nil {
		return false, err
	}
...
func (c *Client) UpdateConfig(ctx context.Context) (bool, error) {
	resp, err := c.SendRequest(ctx, c.clientConfig.RestAPIURL, api.Request{Type: api.GetConfig})
	if err != nil {
...
func (c *Client) RestAPIURL() string {
	return c.clientConfig.RestAPIURL
}
